...
static GIsiPhonetNetlink *nllink = NULL;
...
static void pn_netlink_link(GIsiPhonetNetlink *self, struct nlmsghdr *nlh)
{
	...
	if (self->index != 0 && self->index != (unsigned)ifi->ifi_index)
		return;

#define UP (0x1 /* IFF_UP */ | 0x10000 /* IFF_LOWER_UP */ | 0x40 /* IFF_RUNNING */)
	...
	if (ifname)
		self->callback(st, ifname, self->opaque);
...
gboolean g_isi_pn_netlink_start(GIsiPhonetNetlinkFunc cb, void *data)
{
	GIOChannel *chan;
	int fd;
	...
	nllink->callback = cb;
	nllink->opaque = data;
	nllink->watch = g_io_add_watch(chan, G_IO_IN|G_IO_ERR|G_IO_HUP,
					pn_netlink_process, nllink);
...
void g_isi_pn_netlink_stop(void)
{
	if (nllink == NULL)
		return;
...
int g_isi_pn_netlink_set_address(uint8_t local)
{
	if (nllink == NULL)
		return -ENODEV;
	...
	return pn_netlink_setaddr(nllink->index, local);
}
